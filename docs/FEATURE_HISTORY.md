...
- [`backend/data/places_curated.json`](backend/data/places_curated.json): regenerated by running `build_divvy.py` with CDP credentials; gains a `cdp_divvy` metadata source entry and ~800 `bike_share` place entries.
...
- [`backend/tests/test_places.py`](backend/tests/test_places.py): two new assertions in `TestCuratedSources` — `test_curated_metadata_lists_divvy` (source key present in metadata) and `test_divvy_stations_loaded_into_index` (city-wide query returns > 200 `bike_share` / `cdp_divvy` entries).
