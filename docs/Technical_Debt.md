...
- **Progress (2026-05-24 — autonomous run, steps 2 + 4 + 5):**
  - **Step 2 (migration) complete.** New [`frontend/src/components/ShareCard.css`](../frontend/src/components/ShareCard.css) holds all static styles (one `.share-card-*` class per logical surface — masthead, title, drop figure, map, stats grid, equiv, footer, data, visit). [`ShareDispatch.jsx`](../frontend/src/components/ShareDispatch.jsx) trimmed from 447 → 240 lines; all 30 prior inline `style={{ }}` blocks consolidated into class names. Three dynamic bits stay inline (stat-grid `gridTemplateColumns`, stat-cell `borderRight` for the last-cell omission, stat-num `fontSize` shrink to 16 px for long values) and are commented inline with the dynamic-only rationale.
  - **Step 4 + 5 (CSP) complete.** [`vite.config.js`](../frontend/vite.config.js) now emits `style-src 'self'` (no `'unsafe-inline'`) AND a CSP3 `style-src-attr 'unsafe-inline'` to keep dynamic inline-attribute styling working across the 10 remaining dynamic sites in the codebase (ShareDispatch x3, RouteFlavorTabs, StepHero, WeeklySummaryPanel, LoadingSkeleton, ExploreCategoryPanel x2, primitives.jsx WFSheet). Verified `dist/index.html` carries the new pair via `grep -o 'style-src[^;]*' dist/index.html` after a production build.
  - **Step 1 (baseline) + step 3 (visual diff) + step 6 (device smoke) PENDING USER.** The catalog's acceptance ("Share-card PNG visually identical to baseline") cannot be verified without a running browser. The catalog's `grep` acceptance ("`style={{` matches only dynamic contexts") DOES pass — verified by inspection. To finish: open the share modal in a desktop browser, export the PNG in both Walking + Wheeled mobility profiles, eyeball against a pre-migration screenshot. If anything diverges visually, the static styles in `ShareCard.css` are byte-for-byte copies of the prior inline values, so the divergence would have to come from cascade-precedence interactions — easy to bisect.
