...
- **CPD parks pin layer.** Same `parks_polygons.json` artifact also feeds the "Public parks" pin category alongside the OSM `leisure=park` entries. When `/explore` clips parks against the isochrone for the heatmap fill, `parks.pins_from_feature_collection` derives one pin per park ∩ isochrone slice at the slice's `representative_point()` — guaranteed inside both the park polygon and the walkshed, so large or oddly-shaped parks (Lincoln, Grant, Jackson, Burnham) surface as a pin in their visible portion rather than vanishing when the true centroid falls outside a tight walkshed. `places.dedupe_osm_parks_against_cpd` suppresses any OSM `parks` entry (subcategory None) within 75 m of a CPD pin AND with name-token Jaccard ≥ 0.5 — dog parks / playgrounds inside a CPD park always survive. CPD pins carry `source: "cdp_parks"` and `address: "<acres> acres"`. The handler always runs the parks clip when the parks category is requested even if `with_heatmaps` excluded the parks fill — the response then nulls the heatmap payload but keeps the pins. Pre-existed as a coverage gap (OSM tags 1,405 Chicago parks vs CPD's 804 authoritative entries; many CPD parks weren't tagged in OSM and were invisible as clickable pins).
...
```
`source` is one of `osm`, `cpl_locations`, `farmers_markets_2013`, `cps_schools`, `cpd_stations`, `cfd_stations`, `cdp_divvy`, `cdp_landmarks`, `cdp_parks` (curated source keys). `cdp_parks` entries are synthesized at request time from the same `parks_polygons.json` artifact that feeds `parks_heatmap` — one pin per park ∩ isochrone slice at the slice's `representative_point()`, deduped against collocated OSM `parks` entries (see "CPD parks pin layer" in Key Design Decisions). `residential_heatmap` is `null` for isochrones with no `landuse=residential` overlap.
