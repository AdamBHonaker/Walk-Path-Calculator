...
│   │                     #   baked into the v3 pickle (FEAT-4); v2 pickle / graphml
...
│   ├── tree_canopy.py    # Sparse-KDE canopy clipper for /explore — emits 3 density bands
...
│   │                             #   artifacts at bake time (FEAT-4); pickle = format_version 3.
...
The response also carries `tree_canopy_heatmap`: a GeoJSON FeatureCollection of up to three density bands (`low` ≥ 0.05, `mid` ≥ 0.15, `high` ≥ 0.40) baked from OpenStreetMap `natural=tree` nodes (via Overpass), each band a unioned MultiPolygon of 50 m cell squares clipped to the isochrone. `null` when the canopy KDE artifact (`backend/data/tree_canopy_kde.json`) is missing or no cells overlap the isochrone. Shape:
```json
"tree_canopy_heatmap": {
